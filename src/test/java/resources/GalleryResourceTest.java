...
class GalleryResourceTest {
    ...
    void setup() {
        try {
            em.createNativeQuery("INSERT INTO \"USER\" (id,email,userName,password,firstName,lastName,role,isActive,regestrationDate,canCreateCategory,isBlocked,yearlyFeePaid) VALUES (300,'gal@t.l','galTest','test1234','Gal','Test','Frischling',true,0,false,false,false)").executeUpdate();
            ...
            em.createNativeQuery("INSERT INTO \"USER\" (id,email,userName,password,firstName,lastName,role,isActive,regestrationDate,canCreateCategory,isBlocked,yearlyFeePaid) VALUES (301,'visitor@t.l','visitorTest','test1234','Visitor','Test','Besucher',true,0,false,false,false)").executeUpdate();
    ...
    @Test @TestSecurity(user="galTest",roles={"Frischling"})
    void update404() {
        given().contentType(ContentType.JSON).body("{\"title\":\"x\",\"url\":\"https://x.com\",\"location\":\"x\",\"date\":\"2026-01-01\"}").put("/gallery/99999").then().statusCode(anyOf(is(404),is(500)));
    }
    @Test @TestSecurity(user="galTest",roles={"Frischling"})
    void delete404() {
        given().delete("/gallery/99999").then().statusCode(anyOf(is(404),is(500)));
    }
}
